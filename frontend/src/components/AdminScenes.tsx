...
export default function AdminScenes({ scenes, locations, busy, run }: { scenes: ActiveEventView[]; locations: LocationView[]; busy: boolean; run: AdminAction }) {
  ...
  return <div className="grid items-start gap-6 lg:grid-cols-2"><Section title="布置校园活动" description="活动立即出现在指定地点，角色会根据兴趣决定是否参与。">
    <form className="space-y-4" onSubmit={e => { e.preventDefault(); void run(() => adminApi.createScene({ ...form, tags: tags.split(/[,，]/).map(t => t.trim()).filter(Boolean) }), '活动已发布').then(ok => { if (ok) { setForm({ ...form, title: '', description: '' }); setTags(''); } }); }}>
      <Field label="活动标题"><input className={fieldClass} required maxLength={30} placeholder="湖畔读书会" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></Field>
      <Field label="活动描述"><textarea className={fieldClass} rows={3} maxLength={120} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></Field>
      ...
      <button className="btn-primary" disabled={busy}>发布活动</button>
    </form></Section><Section title="活动列表">{scenes.length === 0 && <p className="text-sm text-muted">还没有校园活动</p>}
    <ul className="space-y-3">{scenes.map(scene => <li key={scene.id} className="rounded-2xl bg-black/[.025] p-4">
      ...
      <p className="my-2 text-sm text-muted">{scene.description}</p><div className="flex items-center justify-between gap-2 text-xs text-muted"><span>{locations.find(l => l.id === scene.location_id)?.name} · #{scene.start_tick}–{scene.end_tick}</span>
        {scene.status !== 'ended' && <button className="btn-outline text-xs" disabled={busy} onClick={() => void run(() => adminApi.endScene(scene.id), '活动已结束')}>结束活动</button>}</div>
    </li>)}</ul></Section></div>;
}
